...
export type FacturePreviewLigne = {
  designation: string;
  description?: string | null;
  quantite: string | number;
  unite: string;
  prixUnitaireHt: number; // centimes
  tauxTva: number; // ×100 (ex: 2000 = 20%)
  remiseHtMontant: number; // centimes
};

export type FacturePreviewData = {
  numero?: string | null;
  titre: string;
  description?: string | null;
  notesClient?: string | null;
  statut: FactureStatutType;
...
type Props = {
  facture: FacturePreviewData;
  pdfMode?: boolean;
};

export function FacturePreviewCard({ facture, pdfMode = false }: Props) {
  ...
  const totalHtNet =
    totalHtBrut - totalRemiseLignes - facture.remiseGlobaleHt;
  const totalTva = Object.values(totauxParTaux).reduce(
    (s, t) => s + t.tva,
    0,
  );
    ...
    >
      ...
      <div className="flex items-start justify-between px-10 pt-8 pb-4">
        ...
        <div>
          ...
          <div className="mt-3 space-y-0.5">
            ...
            {(facture.periodeDebut || facture.periodeFin) && (
              <div className="flex items-center gap-2">
                <span className="w-28 text-xs text-gray-500">Période</span>
                <span className="text-xs font-medium">
                  {facture.periodeDebut
                    ? formatDate(facture.periodeDebut)
                    : "—"}{" "}
                  →{" "}
                  {facture.periodeFin
                    ? formatDate(facture.periodeFin)
                    : "—"}
                </span>
              </div>
            )}
        ...
        <div className="flex flex-col items-end gap-2">
          {facture.emetteurLogoUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={facture.emetteurLogoUrl}
              alt={facture.emetteurEntrepriseNom}
            ...
            <div
              ...
              style={{ fontSize: "18pt" }}
            >
              {facture.emetteurEntrepriseNom.charAt(0).toUpperCase()}
            </div>
          )}
        </div>
      </div>
      ...
      <div
        className={`grid grid-cols-2 gap-0 px-10 ${pdfMode ? "py-2" : "py-3"}`}
      >
        {/* Émetteur */}
        <div
          className={`border-r border-gray-200 pr-6 ${pdfMode ? "[&_p]:!leading-snug [&_p]:!mb-0.5" : ""}`}
        >
          <div
            className={`text-primary text-xs font-semibold tracking-wider uppercase ${pdfMode ? "mb-4" : "mb-2"}`}
          >
            De
          </div>
          ...
          <div className={pdfMode ? "" : "mt-1 space-y-0.5"}>
            {(facture.emetteurPrenomContact ||
              facture.emetteurNomContact) && (
            ...
            {facture.emetteurPhoneContact && (
              <p className="text-xs text-gray-600">
                {facture.emetteurPhoneContact}
              </p>
            )}
            <p className="text-xs text-gray-500">
              SIRET : {facture.emetteurEntrepriseSiret}
            </p>
            {facture.emetteurEntrepriseNumeroTva && (
              <p className="text-xs text-gray-500">
                N° TVA : {facture.emetteurEntrepriseNumeroTva}
              </p>
            )}
          </div>
        </div>
        ...
        {/* Destinataire */}
        <div
          className={`pl-6 ${pdfMode ? "[&_p]:!leading-snug [&_p]:!mb-0.5" : ""}`}
        >
          <div
            className={`text-primary text-xs font-semibold tracking-wider uppercase ${pdfMode ? "mb-4" : "mb-2"}`}
          >
          ...
          <div className={pdfMode ? "" : "mt-1 space-y-0.5"}>
            {facture.destinataireEntrepriseSiret && (
              <p className="text-xs text-gray-500">
                SIRET : {facture.destinataireEntrepriseSiret}
              </p>
            )}
          ...
          <tbody>
            {lignes.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="py-8 text-center text-xs italic text-gray-400"
                >
                  Aucune ligne
                </td>
              </tr>
              ...
              lignes.map((l, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className="border-b border-gray-100 px-2 py-2 text-gray-600"
                      style={{ textAlign: "center" }}
                    >
                      {l.tauxTva / 100} %
                    </td>
                    <td
                      className="border-b border-gray-100 px-3 py-2 font-medium"
                      style={{ textAlign: "right" }}
                    >
                      {l.remiseHtMontant > 0 ? (
                        <span>
      ...
      <div className="mt-4 mb-8 flex justify-end px-10">
        <div style={{ width: "60mm" }}>
          ...
          {facture.remiseGlobaleHt > 0 && (
            <div className="flex justify-between border-b border-gray-100 py-1 text-xs text-red-600">
              <span>Remise globale</span>
              <span>−{formatEur(facture.remiseGlobaleHt)}</span>
            </div>
          )}

          {(totalRemiseLignes > 0 || facture.remiseGlobaleHt > 0) && (
      ...
      {/* ═══ NOTES CLIENT ═══ */}
      {facture.notesClient && (
        <div className="mx-10 mb-6 rounded border border-gray-200 bg-gray-50 px-4 py-3">
      ...
      {facture.statut === "emise" && (
        <div className="mx-10 mb-6 rounded border-2 border-blue-500 py-3 text-center">
      ...
      {facture.statut === "litige" && (
        <div className="mx-10 mb-6 rounded border-2 border-orange-400 py-3 text-center">
      ...
      {/* ═══ PIED DE PAGE ═══ */}
      <div
        className={`border-primary/20 bg-primary/[0.06] mt-auto border-t-2 px-10 text-center text-xs text-gray-500 ${pdfMode ? "py-2 [&_p]:!leading-snug [&_p]:!mb-0.5" : "py-4"}`}
      >
        <p className="font-medium">{facture.emetteurEntrepriseNom}</p>
        <p>SIRET {facture.emetteurEntrepriseSiret}</p>
        {facture.emetteurEntrepriseNumeroTva && (
          <p>N° TVA : {facture.emetteurEntrepriseNumeroTva}</p>
        )}
      </div>
    </div>
  );
