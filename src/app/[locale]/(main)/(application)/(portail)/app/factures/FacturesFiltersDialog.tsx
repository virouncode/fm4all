...
type FacturesFiltersDialogProps = {
  open: boolean;
...
export function FacturesFiltersDialog({
  open,
  onOpenChange,
  currentFilters,
  onApply,
  posture,
  tabType,
}: FacturesFiltersDialogProps) {
  const entreprise = useAppStore((state) => state.entreprise);

  // Posture client : destinataire = lui-même, jamais de filtre client/emetteur
  // Posture prestataire émises : destinataire (client) est filtrable, emetteur masqué
  ...
  const showEmetteurFilter =
    ...
    posture === "client" ||
    posture === "plateforme";
  const showModeFilter = posture === "plateforme" || posture === "prestataire";
  ...
  const form = useForm<FiltersType>({
    ...
    defaultValues: {
      ...
      modeCommercialSnapshot: currentFilters.modeCommercialSnapshot || "all",
  ...
  // Load clients for prestataire-emises / plateforme
  useEffect(() => {
  ...
  // Load sites — client: ses sites / prestataire-emises|plateforme: sites du client sélectionné
  useEffect(() => {
  ...
  // Reset siteId when client changes
