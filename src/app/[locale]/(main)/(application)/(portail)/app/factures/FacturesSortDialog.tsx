...
export function FacturesSortDialog({
  open,
  onOpenChange,
  searchParams,
  sortOptions,
  tabType,
}: FacturesSortDialogProps) {
  ...
  const handleSortChange = (orderBy: string) => {
    router.replace({
      pathname: "/app/factures",
      query: { ...searchParams, tab: tabType, orderBy },
    });
  };
  ...
  const handleDirectionChange = (orderDir: string) => {
    router.replace({
      pathname: "/app/factures",
      query: { ...searchParams, tab: tabType, orderDir },
    });
  };
