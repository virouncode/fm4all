...
import { inviterPrestataireAdminAction } from "@/server/actions/clientServiceExecutionsActions";
import { Mail } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

type InviterPrestataireDialogProps = {
  open: boolean;
...
export function InviterPrestataireDialog({
  open,
  onOpenChange,
  clientEntrepriseId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const result = await inviterPrestataireAdminAction({
      clientEntrepriseId,
      prestataireEntrepriseId,
      email: email.trim(),
    });
      ...
      <DialogContent className="max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <p className="text-muted-foreground text-sm">
            Envoyer une invitation à <strong>{prestataireNom}</strong> pour
