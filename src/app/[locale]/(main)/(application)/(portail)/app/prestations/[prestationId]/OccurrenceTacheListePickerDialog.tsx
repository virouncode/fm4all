...
import { updateOccurrenceTacheListeAction } from "@/server/actions/clientServiceOccurrencesActions";
...
type OccurrenceTacheListePickerDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Fourni uniquement en mode édition — absent lors de la création */
  occurrenceId?: string;
  prestationId: string;
  serviceId: string;
  serviceNom?: string;
...
export function OccurrenceTacheListePickerDialog({
  open,
  onOpenChange,
  occurrenceId,
  prestationId,
  ...
  currentPackId,
  onSuccess,
}: OccurrenceTacheListePickerDialogProps) {
  ...
  const handleSave = async () => {
    if (occurrenceId) {
      ...
      const result = await updateOccurrenceTacheListeAction({
        occurrenceId,
        prestationId,
        entrepriseId,
      ...
      toast.success(
        selectedPackId
          ? "Checklist assignée à l'intervention."
          : "Checklist de l'intervention retirée.",
      );
    }
        ...
        <DialogStyledHeader>
          <DialogHeader>
            ...
            <p className="text-muted-foreground text-xs">
              Override la checklist de la règle / exécution pour cette intervention spécifique.
            </p>
          </DialogHeader>
        </DialogStyledHeader>
          ...
          {loading ? (
            ...
            <ScrollArea className="h-[50vh] px-5 py-4">
              <div className="space-y-2">
                {/* Option "Hériter de la règle / exécution" */}
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      Hériter de la règle / exécution (par défaut)
                    </span>
                    {selectedPackId === null && (
