...
const MesLocaux = () => {
  const t = useScopedI18n("mesLocaux");
  const tErrors = useScopedI18n("mesLocauxFormErrors");
  const tBuildingTypes = useScopedI18n("buildingTypes");
  const tOccupationTypes = useScopedI18n("occupationTypes");
  ...
  const localizedMesLocauxSchema = useMemo(() => {
    return createMesLocauxSchema({
      surfaceRange: tErrors("surfaceRange"),
      staffRange: tErrors("staffRange"),
  ...
  const form = useForm<MesLocauxType>({
    mode: "onBlur",
    resolver: zodResolver(localizedMesLocauxSchema),
    defaultValues,
  });
  ...
  const submitForm = async (data: MesLocauxType) => {
    ...
    try {
      ...
      if (cityData.length === 0) {
        ...
        toast({
          ...
          description: t("invalidPostalCodeDescription"),
        });
        return;
      }
    ...
    if (
      ...
      setDevisProgress({ ...devisProgress, completedSteps: [] });
      router.push("/fr/zone-non-couverte?destination=/");
      return;
    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:gap-8">
          <div className="w-full md:w-1/2 flex flex-col gap-4">
            <InputWithLabel<InsertClientType>
              fieldTitle={t("postalCode")}
              nameInSchema="codePostal"
              placeholder="XXXXX"
            />
            <InputWithLabel<InsertClientType>
              fieldTitle={t("surface")}
          ...
          <div className="w-full md:w-1/2 flex flex-col gap-4 ">
            <SelectWithLabel<InsertClientType>
              fieldTitle={t("buildingType")}
              nameInSchema="typeBatiment"
              data={batiments.map((item) => {
                // Utiliser directement la description pour éviter les problèmes de typage
                return {
                  id: item.id,
                  description:
                    item.id === "bureaux"
                      ? tBuildingTypes("bureaux")
                      : item.id === "localCommercial"
                        ? tBuildingTypes("localCommercial")
                        : item.id === "entrepot"
                          ? tBuildingTypes("entrepot")
                          : tBuildingTypes("cabinetMedical"),
                };
              })}
              handleSelect={handleSelect}
            />
            <SelectWithLabel<InsertClientType>
              fieldTitle={t("occupationType")}
              ...
              data={occupations.map((item) => {
                ...
                return {
                  ...
                  description:
                    item.id === "partieEtage"
                      ? tOccupationTypes("partieEtage")
                      : item.id === "plateauComplet"
                        ? tOccupationTypes("plateauComplet")
                        : tOccupationTypes("batimentEntier"),
                };
              })}
              handleSelect={handleSelect}
            />
        ...
        {devisProgress.completedSteps.includes(1) ? (
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px] w-5/6 lg:w-auto rounded-xl">
              <DialogHeader>
                <DialogTitle>{t("quoteInProgress")}</DialogTitle>
                <DialogDescription>
                  {t("quoteInProgressDescription")}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <DialogClose asChild>
