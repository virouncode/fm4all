...
export const clientServiceOccurrences = pgTable(
  "client_service_occurrences",
  {
    ...
    // Nom déclaré librement par un agent terrain sans compte utilisateur
    assigneeNom: varchar("assignee_nom", { length: 150 }),
...
export const occurrenceTaches = pgTable(
  "occurrence_taches",
  {
    ...
    // Noms déclarés librement par un agent terrain sans compte utilisateur
    assigneeNom: varchar("assignee_nom", { length: 150 }),
    completeeParUserId: uuid("completee_par_user_id").references(
      () => user.id,
      { onDelete: "set null" },
    ),
    completeeParNom: varchar("completee_par_nom", { length: 150 }),
...
// ---------------------------------------------------------------------------
// LIENS TERRAIN (accès sans compte utilisateur)
// ---------------------------------------------------------------------------

/**
 * Lien d'accès terrain généré pour une occurrence.
 * Partageable — plusieurs agents peuvent l'utiliser, chacun crée sa propre session.
 * Généré automatiquement à J-1 par le cron, ou manuellement depuis le back-office.
 */
export const occurrenceFieldLinks = pgTable(
  "occurrence_field_links",
  {
    id: id(),
    occurrenceId: uuid("occurrence_id")
      .notNull()
    ...
    // Qui a généré ce lien (null si généré par le cron)
    createdById: createdById(() => user),
  ...
  (t) => [
    index("ofl_occurrence_idx").on(t.occurrenceId),
    index("ofl_token_idx").on(t.token),
  ],
);

/**
 * Session terrain ouverte par un agent via un occurrenceFieldLink.
 * Trace qui a réalisé l'intervention même sans compte utilisateur.
 */
export const occurrenceFieldSessions = pgTable(
  "occurrence_field_sessions",
  {
    id: id(),
    fieldLinkId: uuid("field_link_id")
      .notNull()
      .references(() => occurrenceFieldLinks.id, { onDelete: "cascade" }),
    // Nom déclaré librement par l'agent au démarrage
    assigneeNom: varchar("assignee_nom", { length: 150 }).notNull(),
  ...
  (t) => [index("ofs_field_link_idx").on(t.fieldLinkId)],
);
